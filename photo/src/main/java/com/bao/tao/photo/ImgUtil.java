...
public class ImgUtil {
  ...
  ImageView imageView;
  @TargetApi(19)
  public  Bitmap handleImageOnKitKat(Context context, Intent data, ImageView imageViews) {
  ...
  //4.4以下系统使用这个方法处理图片 
  public  Bitmap handleImageBeforeKitKat(Context context, Intent data) {
    Uri uri = data.getData(); 
  ...
  public  Bitmap getImage(String srcPath) {
    ...
    int bi = Math.max(outWidth/width  , outHeight/ height );
    options.inSampleSize = bi;
    options.inJustDecodeBounds = false;
    Bitmap bitmap1 = BitmapFactory.decodeFile(srcPath, options);
   ...
   /* BitmapFactory.Options newOpts = new BitmapFactory.Options();
    ...
    int width = imageView.getWidth();
    int height = imageView.getHeight();
    int b = Math.max(outWidth / width, outHeight / height);
